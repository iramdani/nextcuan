...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    let updatedFunc = `function renderSalesChart(periodDays) {
            ...
            adminSalesChartInstance = new Chart(ctx, {
                ...
                options: {
                    layout: { padding: { left: 0, right: 10, top: 40, bottom: 10 } },
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: { display: true, position: 'top', align: 'end', labels: { color: '#94a3b8', font: { family: "'Plus Jakarta Sans', sans-serif", size: 11, weight: 'bold' }, boxWidth: 8, usePointStyle: true } },
                        tooltip: { backgroundColor: 'rgba(10, 10, 24, 0.95)', titleFont: { size: 14, weight: 'bold' }, bodyFont: { size: 13 }, padding: 12, cornerRadius: 8, displayColors: true, borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, callbacks: { title: function(context) { if(context && context.length > 0) { let originalIso = sortedDates[context[0].dataIndex]; let parts = originalIso.split('-'); return 'Tanggal: ' + parts[2] + '/' + parts[1] + '/' + parts[0]; } return ''; }, label: function(context) { let label = context.dataset.label || ''; let revStr = new Intl.NumberFormat('id-ID', { maximumFractionDigits: 0 }).format(context.parsed.y); let countStr = context.dataset.orderCountData[context.dataIndex]; return label + ': Rp ' + revStr + ' (' + countStr + ' Pesanan)'; } } }
                    },
                    scales: {
                        x: { offset: false, grid: { display: false, drawBorder: false }, ticks: { align: 'inner', font: { size: 11 }, color: '#64748b' } },
                        y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,0.03)', drawBorder: false }, ticks: { font: { size: 11 }, color: '#64748b', callback: function(value) { if(value >= 1000000) return 'Rp' + (value/1000000) + 'jt'; if(value >= 1000) return 'Rp' + (value/1000) + 'k'; return 'Rp' + value; }, maxTicksLimit: 6 } }
                    },
