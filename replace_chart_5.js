    ...
    let updatedFunc = `function renderSalesChart(periodDays) {
            ...
            adminSalesChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: labels,
                    datasets: [
                        { label: 'Lunas', data: dataRevLunas, orderCountData: dataCountLunas, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', borderWidth: 3, pointBackgroundColor: '#000018', pointBorderColor: '#10b981', pointBorderWidth: 2, pointRadius: 4, pointHoverRadius: 6, fill: true, tension: 0.4 },
                        { label: 'Pending', data: dataRevPending, orderCountData: dataCountPending, borderColor: '#f59e0b', backgroundColor: 'rgba(245, 158, 11, 0.1)', borderWidth: 3, pointBackgroundColor: '#000018', pointBorderColor: '#f59e0b', pointBorderWidth: 2, pointRadius: 4, pointHoverRadius: 6, fill: true, tension: 0.4 },
                        { label: 'Batal', data: dataRevBatal, orderCountData: dataCountBatal, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderWidth: 3, pointBackgroundColor: '#000018', pointBorderColor: '#ef4444', pointBorderWidth: 2, pointRadius: 4, pointHoverRadius: 6, fill: true, tension: 0.4 }
                    ]
                },
                options: {
                    layout: { padding: { left: 0, right: 0, top: 40, bottom: 20 } }, // Beri ruang atas/bawah tapi 0 kiri/kanan
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: { display: true, position: 'top', align: 'end', labels: { color: '#94a3b8', font: { family: "'Inter', sans-serif", size: 11, weight: 'bold' }, boxWidth: 10, boxHeight: 10, usePointStyle: true } },
                        tooltip: { backgroundColor: 'rgba(5, 5, 20, 0.95)', titleFont: { family: "'Inter', sans-serif", size: 14, weight: 'bold' }, bodyFont: { family: "'Inter', sans-serif", size: 13, weight: '500' }, padding: 12, cornerRadius: 8, displayColors: true, borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1, callbacks: { title: function(context) { if(context && context.length > 0) { let originalIso = sortedDates[context[0].dataIndex]; let parts = originalIso.split('-'); return 'Tanggal: ' + parts[2] + '/' + parts[1] + '/' + parts[0]; } return ''; }, label: function(context) { let label = context.dataset.label || ''; let revStr = new Intl.NumberFormat('id-ID', { maximumFractionDigits: 0 }).format(context.parsed.y); let countStr = context.dataset.orderCountData[context.dataIndex]; return label + ': Rp ' + revStr + ' (' + countStr + ' Pesanan)'; } } }
                    },
                    scales: {
                        x: { 
                            offset: false,
                        ...
                        y: { 
                            ...
                            ticks: { font: { family: "'Inter', sans-serif", size: 11 }, color: '#64748b', callback: function(value) { if(value >= 1000000) return 'Rp ' + (value/1000000).toFixed(1).replace(/\\\\.0$/, '') + 'jt'; if(value >= 1000) return 'Rp ' + (value/1000).toFixed(1).replace(/\\\\.0$/, '') + 'k'; return 'Rp ' + value; }, maxTicksLimit: 6 } 
                        }
                    },
